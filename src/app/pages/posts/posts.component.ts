...
export class PostsComponent implements OnInit {
  ...
  constructor(public userService:UserService, private router:Router, private storage:AngularFireStorage, public postsService:PostsService, private snackbar:MatSnackBar) { }
  ...
  ngOnInit(): void {
    ...
    this.postsService.getPosts().then((res:any)=>{
      this.posts = res;
  ...
  post(){
    this.snackbar.open('Posting...', '', {duration:2000})
    if(this.selectedFile != undefined || this.selectedFile != null){
      this.uploadImage().then((imageURL)=>{
        ...
        this.postsService.saveNewPost(postObj).then((res)=>{
          console.log(res);
          this.snackbar.open('Posted Successfully', 'OK', {duration:2000});
        }).catch((err)=>{
          console.log(err);
        });
  ...
  like(postId:any){
    for(let i = 0; i < this.posts.length; i++){
      if (this.posts[i].id == postId){
        ...
        this.postsService.updateLikes(this.posts[i]).then((res)=>{
          console.log(res);
        }).catch((err)=>{
          console.log(err);
        })
  ...
  comment(postId:any, commentIndex:any){
    for(let i = 0; i < this.posts.length; i++){
      if(this.posts[i].id == postId){
        ...
        this.postsService.updateComments(this.posts[i]);
      }
    }
